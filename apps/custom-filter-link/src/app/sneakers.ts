...
const filterByBrand = (brand: string) =>
  SNEAKERS.filter((x) => x.brand.toLowerCase() === brand.toLowerCase());

const getSneakerById = (id: string) =>
  SNEAKERS.find((sneaker) => sneaker.id === id);

const brands = [...new Set(SNEAKERS.map((x) => x.brand))];
